/*
抓bbyb.yichengwangluo.net中请求包的Authorization和device

青龙格式export bbybapp='Authorization#device'

cron: 1 6 * * *

*/
const $ = new Env("步步有宝");   // 1.名字改了
const ckName = "bbybapp";           // 2. 英文名字改一下
const { log } = console;
//-------------------- 一般不动变量区域 -------------------------------------      // 3. 不用管 
...
let envSplitor = ["@", "\n"];
let ck = msg = ''
let hosts='https://bbyb.yichengwangluo.net'
...
async voiceindex1() {
  ...
  let sign=MD5Encrypt(`${this.logid}${this.icpm}43514559`)
  let path=`/api/v2/voice/index`
          let urlobj = {
              url: `${hosts}${path}`,
              headers: this.herget,
...
async voiceindex2() {
  ...
  let sign=MD5Encrypt(`${this.tid}${this.icpm}43514559`)
  let path=`/api/v2/voice/index`
          let urlobj = {
              url: `${hosts}${path}`,
              headers: this.herget,
...
async exchange1() {
  let path=`/api/v2/cash/exchange`
          let urlobj = {
              url: `${hosts}${path}`,
              headers: this.herpost,
              body:`gate=wechat&amount=1&lat=&lng=&root=0&sim=1&debug=0&model=&power=0&vpn=0`
          }
          let result = await httpPost(urlobj)
          if(result.code ==0){
            console.log(`账号[${this.name}]：${result.result.message}`)

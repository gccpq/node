/*
抓ykd.yichengwangluo.net中请求包的Authorization和device

青龙格式export ykdapp='Authorization#device'


cron: 1 6 * * *
*/
const $ = new Env("阅看点");   // 1.名字改了
const ckName = "ykdapp";           // 2. 英文名字改一下
const { log } = console;
//-------------------- 一般不动变量区域 -------------------------------------      // 3. 不用管 
...
let envSplitor = ["@", "\n"];
let ck = msg = ''
let hosts='https://ykd.yichengwangluo.net'
...
async voiceindex1() {
  ...
  let sign=MD5Encrypt(`${this.logid}${this.icpm}43514366`)
  let path=`/api/v2/voice/index`
          let urlobj = {
              url: `${hosts}${path}`,
              headers: this.herget,
...
async voiceindex2() {
  ...
  let sign=MD5Encrypt(`${this.tid}${this.icpm}43514366`)
  let path=`/api/v2/voice/index`
          let urlobj = {
              url: `${hosts}${path}`,
              headers: this.herget,

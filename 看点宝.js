/*
抓kbd.yichengwangluo.net中请求包的Authorization和device

青龙格式export kbdapp='Authorization#device'

cron: 1 6 * * *

*/
const $ = new Env("看点宝");   // 1.名字改了
const ckName = "kdbapp";           // 2. 英文名字改一下
const { log } = console;
//-------------------- 一般不动变量区域 -------------------------------------      // 3. 不用管 
...
let envSplitor = ["@", "\n"];
let ck = msg = ''
let hosts='https://kdb.yichengwangluo.net'
...
async voiceindex1() {
  ...
  let sign=MD5Encrypt(`${this.logid}${this.icpm}43514368`)
  let path=`/api/v2/voice/index`
          let urlobj = {
              url: `${hosts}${path}`,
              headers: this.herget,
...
async voiceindex2() {
  ...
  let sign=MD5Encrypt(`${this.tid}${this.icpm}43514368`)
  let path=`/api/v2/voice/index`
          let urlobj = {
              url: `${hosts}${path}`,
              headers: this.herget,
